...
enum class OtaPhase : uint8_t { PENDING=0, ACK=1, STARTING=2, FAIL=3, SUCCESS=4 };

struct OtaStatusPayload {
  uint8_t   stationId;
  uint8_t   stationType;     // StationType (e.g., LOOT)
  uint32_t  campaignId;
...
// New: targeted control payload
// targetType: 0 = ALL types, else StationType (TREX, LOOT, DROP, CONTROL)
// targetId:   0 = ALL ids of that type, else specific stationId
struct ControlCmdPayload {
  uint8_t op;         // ControlOp
